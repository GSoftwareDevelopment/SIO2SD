...
class FakeSD:
    def __init__(self):
        ...
        self.chg = 0
        self.hsindex = 16      # dzielnik POKEY turbo (~38400)
        self.turbo_ok = True   # czy urzadzenie odpowiada w turbo
    ...
    def cmd(self, c, aux1, aux2, data):
        ...
        if c == 0x1E: return 1, bytes([self.hsindex])
        if c == 0x09:
...
def call(addr, a=0, x=0, y=0, maxstep=8_000_000):
    ...
    while mpu.pc != RET:
        if mpu.pc == 0xCFED:
            raise AssertionError('SDCDEV /H nie powinien skakac pod $CFED')
        if mpu.pc == SIOV:
            sio_call(); continue
...
if HISIO:
    for i, ch in enumerate('Hias 1.33 230101'):
        mem[0xCFF0 + i] = ord(ch)
    print('(tryb HISIO: sygnatura Hias obecna, ale /H uzywa bitu $80)')
...
def reinstall(cmdline):
    ...
    mem[SYMS['INSTALL']] = 0
    for nm in ('nslots','i_mode','i_id','i_any','i_try','hsopt','siomode'):
...
print('--- opcja /H (turbo SIO) ---')
sd.turbo_ok = True
reinstall('/H')
check(mem[lab('siomode')] == 0x80,
      f'/H: turbo wlaczone przez bit $80, siomode=$80: {mem[lab("siomode")]:02X}')
check(any('turbo wlaczone' in x for x in printed), f'komunikat turbo ON: {printed}')
sd.turbo_ok = False
reinstall('/H')
check(mem[lab('siomode')] == 0,
      f'/H: turbo nie dziala -> 19200 (siomode=0): {mem[lab("siomode")]:02X}')
check(any('niedostepne' in x for x in printed), f'komunikat fallback: {printed}')
sd.turbo_ok = True
reinstall('')
check(mem[lab('siomode')] == 0, f'bez /H: siomode=0: {mem[lab("siomode")]:02X}')
